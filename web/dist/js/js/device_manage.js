$(function(){
    ...
    // TODO
    $(".sensor-wra").on("mouseover", ".col-xs-4", function() {
      $(this).css("background","#f0f2fd");
      $(this).children("div.right").css("display","none");
      $(this).children("div.setup").css("display","block");
    })
    $(".sensor-wra").on("mouseout", ".col-xs-4", function() {
      $(this).css("background","#fff");
      $(this).children("div.right").css("display","");
      $(this).children("div.setup").css("display","none");
    })
...
//显示控制设备页面
function showKzDeviceDiv(id,name,num){
	$("#kz_device_num").val(id);
	$("#kz_device_name").html(name + num + '设置');
  $("#kz_device_div").attr("data-num", num);
	kzDeviceLeftData(id, num);
}
function kzDeviceLeftData(id,num){
  var num;
  if (!num) num = 0;
  sessionStorage.setItem("switchNum_",num);
  // var loginInfo = JSON.parse(sessionStorage.getItem("loginInfo"));
	$.ajax({
		type:"POST",
	 		url: URLAPI + "actionServlet",
  ...
  // var loginInfo = JSON.parse(sessionStorage.getItem("loginInfo"));
	...
	if(model == '0'){
	 ...
	 $.ajax({
      ...
      "loopWeek":week,
      "useState":useState,
      "switchNum": $("#kz_device_div").attr("data-num"),
      "companyId": loginInfo.id,
      "userType": type_,
      "token": getCookie("token")
    },
        ...
        var num = sessionStorage.getItem("switchNum_");
 				kzDeviceLeftData(id, num);
 				swal({
 	    	  		title: "保存成功",
 	          		text: "",
...
// 显示模态框  -- 天窗 TODO
function showWin(id, state, name){
  // var loginInfo = JSON.parse(sessionStorage.getItem("loginInfo"));
  // $(".shezhi-tt-con li:eq(0)").html(name);
  // $("#shezhi-1").val(id);
  // $("#shezhi-1").attr("data-state", state);
  $(".shezhi-tt-head span").html(name);
  $("#shezhi-tt").modal('show');
  if (name == "投食开关") {
    $(".toushi").show();
  }else{
    $(".toushi").hide();
    ...
    success: function (msg) {
      ...
      if (msg.statusCode == 1) {
        ...
        msg.result.forEach(function(item, index) {
          var index = index + 1;
          str += '<ul class="shezhi-tt-con"><li data-name="'+name+'">'+name+item.num+'</li>'
          		 + '<li><input onchange="conDeviceOpenClose22(this)" type="checkbox" class="shezhi_" id="shezhi-'+index+'" name="name" data-state='+item.state+' data-index='+index+' data-val='+id+'></li>'
          		 + '<li><span class="shezhi-btn">设置</span></li></ul>'
        })
        $(".shezhi-tt-content").html(str);
        initializeBtn();
      } else {
        toastr.error('请求数据出错','请联系管理员');
      }
    }
  })
}
function hideShezhi() {
  $("#shezhi-tt").modal('hide');
}
// 初始化设置按钮
// initializeBtn();
function initializeBtn(state){
  var showIcon = $(".controller-items").attr("data-show");
  if(showIcon == 1 || showIcon == "1") {
    //var shezhi1 = document.querySelector('#shezhi-1')
    // var shezhi2 =camera_con = document.querySelector('#shezhi-2')
    //var shezhi11 = new Switchery(shezhi1,{"color": "#1885dc","secondaryColor": "#f00"});
    // var shezhi22 = new Switchery(shezhi2,{"color": "#1885dc","secondaryColor": "#f00"});
  }
    var length = $(".shezhi-tt-con").length + 1;
    for (var i = 1; i < length; i++) {
      var state_ = $('#shezhi-' + i).attr('data-state') == 1 ? true : false;
      // var state_ = $('#shezhi-' + i).attr('data-state');
      var shezhi11;
      if (state_) {
        shezhi11 = new Switchery($('#shezhi-'+i)[0],{"color": "#1885dc","secondaryColor": "#f00"});
      } else {
        shezhi11 = new Switchery($('#shezhi-'+i)[0],{"color": "#1885dc","secondaryColor": "#f00"});
      }
      setSwitchery(shezhi11, state_)
    }

    // setSwitchery(shezhi11, state)
    $(".controller-items").attr("data-show",0);

}

// 投食机推送
function feedingMachinePush() {
