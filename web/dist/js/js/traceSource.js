...
function getSourceList(page){
	...
	$.ajax({
 		...
 		url: URLAPI + "actionServlet",
 		data:{
			"method":"queryProducts","actionName":"sourceService"
...
function showAreaMap(){
	...
	$.get('../dist/js/echarts/china.json', function (chinaJson) {
		...
		$.ajax({
	 		...
	 		url: URLAPI + "actionServlet",
	 		data:{
				"actionName":"sourceService","method":"scanArea",
				"companyId": loginInfo.id,
				"userType": loginInfo.type,
				"token": getCookie("token")
			},
...
function saveNewProduct(){
	...
	$.ajax({
 		...
 		url: URLAPI + "actionServlet",
 		data:{
			"actionName":"sourceService","method":"addProduct"
...
function saveBatch(){
	...
	if(batchId == '0'){
		...
		$.ajax({
	 		...
	 		url: URLAPI + "actionServlet",
	 		data:{
				"actionName":"sourceService","method":"updateBatch","batchId":batchId
		...
		swal({
			   ...
			   cancelButtonText: '取消',
				 ...
				 $.ajax({
				 		...
				 		url: URLAPI + "actionServlet",
				 		data:{
							"actionName":"sourceService","method":"shelves","batchIds":selectIds,
							"companyId": loginInfo.id,
							"userType": loginInfo.type,
							"token": getCookie("token")
						},
		...
		swal({
			   ...
			   cancelButtonText: '取消',
				 ...
				 $.ajax({
				 		...
				 		url: URLAPI + "actionServlet",
				 		data:{
							"actionName":"sourceService","method":"offShelf","batchIds":selectIds,
							"companyId": loginInfo.id,
							"userType": loginInfo.type,
							"token": getCookie("token")
						},
		...
		swal({
			   ...
			   cancelButtonText: '取消',
				 ...
				 $.ajax({
				 		...
				 		url: URLAPI + "actionServlet",
				 		data:{
							"actionName":"sourceService","method":"deleteBatch","ids":selectIds,"productId":productId,
							"companyId": loginInfo.id,
							"userType": loginInfo.type,
							"token": getCookie("token")
						},
...
function updateBatchInfo(batchId){
	...
	setTimeout(function () {
		$.ajax({
	 		...
	 		url: URLAPI + "actionServlet",
	 		data:{
				"actionName":"sourceService","method":"getBatchById","id":batchId,
				"companyId": loginInfo.id,
				"userType": loginInfo.type,
				"token": getCookie("token")
			},
...
//	console.info("自定义模块" + JSON.stringify(customJsonArr));
	...
	$.ajax({
 		...
 		url: URLAPI + "actionServlet",
 		data:{
			"actionName":"sourceService","method":"updateArchive","productId":sourceId
...
function preview(barcode){
	...
	$.ajax({
 		type:"POST",
 		url: URLAPI + "staticServlet",
 		data:{
			"actionName":"STCH5Service",
			"method":"showArchive",
			"barcode":barcode,
			"companyId": loginInfo.id,
			"userType": loginInfo.type,
...
function getEnvData(obj){
   ...
   $.ajax({
 		type:"POST",
 	// 	url: URLAPI + "wisdom_farming_site/staticServlet",
 		url: URLAPI + "staticServlet",
 		data:{
			"actionName":"STCH5Service","method":"queryEnvData","batchId":batchId,"deviceNum":deviceId,
			"companyId": loginInfo.id,
			"userType": loginInfo.type,
			"token": getCookie("token")
		},
