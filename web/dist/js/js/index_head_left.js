...
var type_index;
if (loginInfo.type_index && loginInfo.type_index == 3) {
  type_index = 2;
} else {
  type_index = loginInfo.type;
}
function header(){
    $("#warning_icon").click(function(){
      //$("#warning_icon .remind").css({'background':"transparent","color":"transparent"});
    	...
    	window.location.href="warningManage.html";
    });
    $("#quit").click(function(){
    	// window.location.href="loginServlet?method=userLogOut";
      sessionStorage.clear();
      setCookie("loginInfo", null);
...
function warningCount(){
	// var loginInfo = JSON.parse(sessionStorage.getItem("loginInfo"));
	 $.ajax({
	    	type:"POST",
			// url:"http://www.nxptdn.com/actionServlet",
      url: URLAPI + "actionServlet",
			data:{
      	"actionName": "earlyWaringService",
      	"method": "queryNotReadCount",
...
$(window).resize(function() {
   ...
   $("#div1").css("height",midHeight-60+'px');
   $('.main').css("min-height",footHeight-60+'px');
	//  $("#div1").css("height",midHeight-50+'px');
	//  $('.main').css("min-height",footHeight+120+'px');
});
...
function menu(){
  //  var loginInfo = JSON.parse(sessionStorage.getItem("loginInfo"));
   var height_item1 = 170;
   var height_item2 = 360;
	 var footHeight = $('#divFooter').offset().top;
   if (footHeight < (height_item1 + height_item2)) {
     // $("#div1").css("height",footHeight-60+'px');
     $('.main').css("min-height",(height_item1 + height_item2)+'px');
   } else {
     $("#div1").css("height",footHeight-60+'px');
     $('.main').css("min-height",footHeight-60+'px');
   }
        ...
        "token": getCookie("token")
          ...
          // console.log(menuData[0]);
                ...
                //12.25
                    ...
                    if ($('#divFooter').offset().top < 650) {
                      $(".main").css("min-height","530px");
                    }else{
                      $('.main').css("min-height",$('#divFooter').offset().top-60+'px');
                    }
					    }else{
                  if ($('#divFooter').offset().top < 650) {
	...
	$("#employee_manage").click(function(){
    	...
    	window.location.href="employeeManage.html";
  });
  // 设备管理
